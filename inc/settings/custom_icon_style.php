...
function word_balloon_custom_icon_style($load_setting){
	...
	foreach ( $load_setting['type_icon'] as $key => $val ):
		...
		?>
		<input id="custom_icon_<?php echo esc_attr( $key ); ?>" type="hidden" name="custom_icon_<?php echo esc_attr( $key ); ?>" data-fill="<?php echo esc_attr( $fill_value ); ?>" data-stroke="<?php echo esc_attr( $stroke_value ); ?>" data-stroke_width="<?php echo esc_attr( $stroke_width_value ); ?>" />
		<?php
	endforeach;
