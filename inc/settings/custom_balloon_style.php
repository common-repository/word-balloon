	...
	foreach ( $load_setting['type_balloon'] as $key => $val ):
		foreach ($direction as $direction_key => $direction_value):
			...
			while($i < 6 ):
				...
				$gradient_color_value .= ' data-gradient_color_'.esc_attr( $i ).'_value="'.esc_attr( ${'gradient_color_' . esc_attr( $i ) . '_value'} ).'"';
				++$i;
			endwhile;
			*/
			?>
			<input id="custom_balloon_<?php echo esc_attr( $key ).'_'.esc_attr( $direction_key ); ?>" type="hidden" name="custom_balloon_<?php echo esc_attr( $key ).'_'. esc_attr( $direction_key ); ?>" data-color_value="<?php echo esc_attr( $color_value ); ?>" data-background_value="<?php echo esc_attr( $background_value ); ?>" data-border_color_value="<?php echo esc_attr( $border_color_value ); ?>" data-border_style_value="<?php echo esc_attr( $border_style_value ); ?>" data-border_width_value="<?php echo esc_attr( $border_width_value ); ?>" data-border_radius_value="<?php echo esc_attr( $border_radius_value ); ?>" data-balloon_shadow_color_value="<?php echo esc_attr( $balloon_shadow_color_value ); ?>" data-avatar_name_position_value="<?php echo esc_attr( $avatar_name_position_value ); ?>" <?php word_balloon_custom_balloon_echo_gradient_color($load_setting,$key,$direction_key); ?>/>
			<?php
		endforeach;
...
function word_balloon_custom_balloon_echo_gradient_color($load_setting,$key,$direction_key){
	$i = 1;
	while($i < 6 ):
		${'gradient_color_' . $i . '_value'} = '';
		if(isset($load_setting['custom_balloon'][$key][$direction_key]['gradient_color_'.$i ])) {
		...
		echo ' data-gradient_color_'.esc_attr( $i ).'_value="'.esc_attr( ${'gradient_color_' . esc_attr( $i ) . '_value'} ).'"';
		++$i;
	endwhile;
}